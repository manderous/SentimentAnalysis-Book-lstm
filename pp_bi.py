...
output_dropout = 0.75 # LSTM输出的dropout rate设置
...
# Unstack to get a list of 'maxSeqLength' tensors of shape (batch_size, num_input)。
# 满足tf.nn.static_bidirectional_rnn的inputs = data的输入格式要求
with tf.name_scope('bi-lstm'):
    data = tf.unstack(data, maxSeqLength, 1)
    # 利用tensorflow环境定义lstm单元
    # Forward direction cell（前向单元）
    lstm_fw_cell = tf.contrib.rnn.BasicLSTMCell(lstmUnits, forget_bias=forget_bias_parameter)
    # Backward direction cell（后向单元）
    lstm_bw_cell = tf.contrib.rnn.BasicLSTMCell(lstmUnits, forget_bias=forget_bias_parameter)
    # 设置双向LSTM的前向和后向的输出dropout rate设置
    lstm_fw_cell = tf.contrib.rnn.DropoutWrapper(cell=lstm_fw_cell, output_keep_prob=output_dropout)
    lstm_bw_cell = tf.contrib.rnn.DropoutWrapper(cell=lstm_bw_cell, output_keep_prob=output_dropout)
    # 创建一个双向循环神经网络
    value, _, _ = tf.nn.static_bidirectional_rnn(lstm_fw_cell, lstm_bw_cell, data, sequence_length=seq_len_ph, dtype=tf.float32)
...
for learning_rate in [1E-4, 1E-5, 1E-6]:
    ...
    with tf.Session() as sess:
        ...
        if learning_rate==1E-4:
            writer = tf.summary.FileWriter("./graphs"+'1E-4', sess.graph) # 选定可视化存储目录
        elif learning_rate==1E-5:
            writer_1 = tf.summary.FileWriter("./graphs"+'1E-5', sess.graph) # learning_rate==1E-5
        else:
            writer_2 = tf.summary.FileWriter("./graphs"+'1E-6', sess.graph) # learning_rate==1E-6
        Loss = 0
        Accuar = 0
